...
export const config = {
    ...
    BOOKING_TTL_SECONDS: Number(process.env.BOOKING_TTL_SECONDS) || 300,
    LOCK_TTL_SECONDS: Number(process.env.LOCK_TTL_SECONDS) || 300,
    INVENTORY_SERVICE_URL: process.env.INVENTORY_SERVICE_URL || "http://localhost:4010/api/v1/inventory",
    INTERNAL_SERVICE_KEY: process.env.INTERNAL_SERVICE_KEY || "internal_secret_key_123",
    PAYMENT_SERVICE_URL: process.env.PAYMENT_SERVICE_URL || "http://localhost:4020/api/v1/payments",
} as const;
